    ...
    try:
        ...
        for i in seat_plans:
            if i["seatPlanId"] in mp:
                mp[i["seatPlanId"]] = (i["seatPlanName"], i["originalPrice"])

        # 符合i["seatPlanName"] in seat_list中最便宜的座位
        for k, (n, p) in mp.items():
            if n in seat_list:
                seat_plan_id = k
                price = p
                break
        ...
        if not deliver_method:
            deliver_method = request.get_deliver_method(
        ...
        if deliver_method == "VENUE_E":
            ...
            if deliver_method == "EXPRESS":
                ...
                # 获取快递费用
                express_fee = request.get_express_fee(
                    show_id,
                    session_id,
                    seat_plan_id,
                    price,
